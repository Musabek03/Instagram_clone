...
def create_comment_notification(sender, instance, created, **kwargs):
    if created:
        if instance.user != instance.post.author:
            Notification.objects.create(sender=instance.user,receiver=instance.post.author,type='comment',post=instance.post,is_read=False)
    ...
    if action == 'post_add':
        for user_id in pk_set:
            ...
            if liker != instance.author:
                Notification.objects.get_or_create(sender=liker,receiver=instance.author,type='like',post=instance,defaults={'is_read': False})
...
def create_follow_notification(sender, instance, action, pk_set, **kwargs):
    if action == 'post_add':
        for follower_id in pk_set:
            ...
            Notification.objects.get_or_create(sender=follower_user,receiver=instance, type='follow',post=None, defaults={'is_read': False})